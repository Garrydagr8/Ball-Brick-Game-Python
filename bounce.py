...
def start_game(event):
    ...
    if playing is False:
        ...
        for i in range(0,5):
            for j in range(0,19):
                canvas.move(bricks[i][j].id,25*j,50*i)
...
canvas.create_text(250,250,text="Press Enter to start Game!!",fill="red",font="Consolas 24")
j=canvas.find_all()
root.mainloop()
